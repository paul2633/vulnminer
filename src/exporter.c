...
FILE *exporter_begin(repository_t *repo, config_t *config) {
    ...
    exit_if(asprintf(&output_path, "%s/%s_%s_%s.json", config->json_path, repo->name, enum_to_string(mode_map, config->mode),
                     enum_to_string(granularity_map, config->granularity)) == -1,
            __func__, "asprintf");
